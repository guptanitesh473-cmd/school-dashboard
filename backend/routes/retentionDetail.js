...
router.get('/', (req, res) => {
  const rows = db.prepare('SELECT * FROM retention_detail ORDER BY sort_order').all();
  const grouped = {};
  for (const r of rows) {
    if (!grouped[r.school_name]) grouped[r.school_name] = [];
    grouped[r.school_name].push(r);
  }
  // Summary totals (exclude Hesarghatta from grand totals as it has different format)
