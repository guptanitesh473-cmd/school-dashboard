...
import {
  ...
  CheckCircle2, Clock, User, Layers, GraduationCap, Inbox, ArrowLeft,
...
const erpApi = {
  ...
  applyLeave: (data) => erpRequest("/leaves", { method: "POST", body: data }),
  getMyLeaves: () => erpRequest("/leaves/mine"),
  getAllLeaves: () => erpRequest("/leaves"),
  approveLeave: (id) => erpRequest(`/leaves/${id}/approve`, { method: "POST" }),
  rejectLeave: (id) => erpRequest(`/leaves/${id}/reject`, { method: "POST" }),
  checkIn: () => erpRequest("/attendance/checkin", { method: "POST" }),
  checkOut: () => erpRequest("/attendance/checkout", { method: "POST" }),
  getMyAttendance: () => erpRequest("/attendance/mine"),
  getAttendanceSummary: () => erpRequest("/attendance/summary"),
};
...
function Attendance() {
  ...
  const load = () => erpApi.getMyAttendance().then(setData).catch((e) => setErr(e.message));
  useEffect(() => { load(); }, []);

  const checkIn = async () => { setErr(""); try { await erpApi.checkIn(); load(); } catch (e) { setErr(e.message); } };
  const checkOut = async () => { setErr(""); try { await erpApi.checkOut(); load(); } catch (e) { setErr(e.message); } };
  ...
  const stats = [
    { label: "Working days elapsed", value: data.workingDaysElapsed, icon: CalendarCheck, color: "#1E2B5C" },
    { label: "Days present", value: data.daysPresent, icon: CheckCircle2, color: "#2E7D5B" },
    { label: "Attendance rate", value: `${data.pct}%`, icon: Clock, color: "#E07C24" },
  ];
...
function Leaves() {
  ...
  const [form, setForm] = useState({ leave_type: LEAVE_BALANCE[0].type, from: "", to: "", reason: "" });
  const [msg, setMsg] = useState("");

  const load = () => erpApi.getMyLeaves().then(setLeaves).catch(() => {});
  useEffect(() => { load(); }, []);
  ...
  const apply = async () => {
    ...
    try {
      await erpApi.applyLeave({ leave_type: form.leave_type, start_date: form.from, end_date: form.to, reason: form.reason.trim() });
      setForm({ leave_type: LEAVE_BALANCE[0].type, from: "", to: "", reason: "" });
      setMsg("Leave applied — sent to admin for approval.");
      load();
    } catch (e) {
      setMsg(e.message);
    }
...
function LeaveRequests() {
  ...
  const load = () => erpApi.getAllLeaves().then(setLeaves).catch(() => {});
  useEffect(() => { load(); }, []);

  const approve = async (id) => { await erpApi.approveLeave(id); load(); };
  const reject = async (id) => { await erpApi.rejectLeave(id); load(); };
...
function AdminDashboard({ employees, pending, pendingLeavesCount, attendanceSummary }) {
  const [expanded, setExpanded] = useState(null);
  const total = employees.length;
  const attendanceFor = (id) => attendanceSummary.find((a) => a.employee_id === id);
  ...
  const stats = [
    ...
    { label: "Pending Leave Requests", value: pendingLeavesCount, icon: FileText, color: "#C4452E" },
  ];
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {employees.map((emp) => {
              ...
              const att = attendanceFor(emp.id);
              return (
                <Fragment key={emp.id}>
...
function MyProgress() {
  ...
  useEffect(() => {
    erpApi.getMyAttendance().then(setAttendance).catch(() => {});
    erpApi.getMyLeaves().then(setLeaves).catch(() => {});
  }, []);
  ...
  const stats = [
    { label: "Attendance rate (this month)", value: attendance ? `${attendance.pct}%` : "—", icon: CalendarCheck, color: "#2E7D5B" },
    { label: "Days present", value: attendance ? attendance.daysPresent : "—", icon: CheckCircle2, color: "#1E2B5C" },
    { label: "Pending leave requests", value: pendingCount, icon: Clock, color: "#E07C24" },
    { label: "Approved leaves", value: approvedCount, icon: FileText, color: "#2E7D5B" },
  ];
...
const BASE_NAV = [
  ...
  { id: "my-progress", label: "My Progress", icon: LayoutDashboard },
...
export default function ERPSystem({ onExit }) {
  ...
  const refreshAdminExtras = () => {
    erpApi.getAllLeaves().then((lv) => setPendingLeavesCount(lv.filter((l) => l.status === "pending").length)).catch(() => {});
    erpApi.getAttendanceSummary().then(setAttendanceSummary).catch(() => {});
  };

  useEffect(() => { if (isAdmin) refreshEmployeeData(); }, [isAdmin]);
  useEffect(() => { if (isAdmin && tab === "admin-dashboard") refreshAdminExtras(); }, [isAdmin, tab]);
  ...
  return (
    <div className="erp min-h-screen flex flex-col md:flex-row" style={{ background: "#FAF9F4" }}>
      ...
      <main className="flex-1 p-5 md:p-10 max-w-5xl w-full mx-auto">
        ...
        {tab === "admin-dashboard" && isAdmin && (
          <AdminDashboard employees={employees} pending={pending} pendingLeavesCount={pendingLeavesCount} attendanceSummary={attendanceSummary} />
        )}
